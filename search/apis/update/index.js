...
module.exports = function(args, finished) {
  ...
  try
  {
    ...
    if (typeof bundle.id !== 'undefined' && bundle.id.length === 0) {
        finished(errorMessage.badRequest(request,'processing', 'fatal', 'Resource ' + bundle.resourceType + ' must have an \'id\' property'));
    }

    if(bundle.id !== searchSetId) {
        finished(errorMessage.badRequest(request,'processing', 'fatal', 'Resource ' + bundle.id + ' does not match ' + searchSetId));
    }

    //Does this bundle exist...
    var searchSet = this.db.use(bundle.resourceType, bundle.id);
    if(!searchSet.exists) {
        finished(errorMessage.notFound(request,'processing', 'fatal', 'Resource ' + bundle.id + ' does not exist'));
    } 
